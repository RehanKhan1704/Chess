...
public class Pawn extends Piece{
    ...
    public boolean isValidMovement(int col,int row){
        ...
        if(isFirstMove && this.col == col && row == this.row - colorIndex * 2 && board.getPiece(col,row)==null && board.getPiece(col,row
                +colorIndex)==null)
            return true;
        ...
        if (board.getTileNum(col,row)==board.enPassant && col == this.col - 1 && row == this.row - colorIndex && board.getPiece(col,row+colorIndex)!=null){
            return true;
        }
        if (board.getTileNum(col,row)==board.enPassant && col == this.col + 1 && row == this.row - colorIndex && board.getPiece(col,row+colorIndex)!=null){
            return true;
